...
def test_no_fire_stays_at_baseline():
    ...
    injector = FireEpisodeInjector([a.id for a in anchors], episode_length_ticks=100, fire_probability=0.0, seed=1)
...
def test_fire_ramps_up_then_back_down():
    ...
    injector = FireEpisodeInjector(
        [a.id for a in anchors], episode_length_ticks=300, fire_probability=1.0, peak_multiplier=10.0, seed=2
    )
